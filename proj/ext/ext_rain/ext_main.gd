...
onready var list_update_event = [
    {"control": $particle_type, "signal_name": "item_selected"},
    ...
    {"control": $fog_enable, "signal_name": "toggled"},
    {"control": $lbl_fog_amount/amount, "signal_name": "value_changed"}, 
    {"control": $lbl_fog_color/fog_color, "signal_name": "color_changed"},
]
...
func _process(_delta):
    ...
    if self.need_update == true:
        ...
        match $particle_type.get_selected_id():
            0:
        ...
        node_r.draw_pass_1.size.y = $lbl_size/size.value
        node_r.amount = $lbl_amount/amount.value
        node_r.process_material.initial_velocity = float($lbl_initial_velocity/initial_velocity.value)

        var radius = $lbl_size/size.value / 64
        node_s.draw_pass_1.radius = radius
        node_s.draw_pass_1.height = radius * 2
        node_s.amount = $lbl_amount/amount.value
        node_s.process_material.initial_velocity = float($lbl_initial_velocity/initial_velocity.value) / 10
        
        node_f.visible = $fog_enable.pressed
        node_f.amount = $lbl_fog_amount/amount.value
        node_f.process_material.color = $lbl_fog_color/fog_color.color
