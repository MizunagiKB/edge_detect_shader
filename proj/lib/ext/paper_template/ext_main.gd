...
func generate_paper():
    ...
    $preview/viewport/paper.MARGIN = self.mm_to_px(
        Vector2($hbox1/margin.value, $hbox1/margin.value)
    )
    $preview/viewport/paper.COMMENT = $hbox2/comment.text
    ...
    if o_image != null:
        ...
        o_image.flip_y()
        o_image.save_png(LibConfigure.capture_image_dir + paper_name)
...
func _ready():
    ...
    $btn_generate.connect("pressed", self, "_on_pressed_btn_generate")
